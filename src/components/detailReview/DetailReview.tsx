...
import{fetchDetailReviewMoreSlice,clearReviewMoreList} from "../../redux/detailReviewSliceCopy/fetchDetailReviewSlice"
...
export const DetailReview: React.FC<pIf> = () => {
    ...
    let initialList = useSelector((state:RootState) => state.detailReviewSliceCopy.initialList);
    let reviewMoreList = useSelector((state:RootState) => state.detailReviewSliceCopy.reviewMoreList);
    
    
    console.log("AAAAAAAAAAAAAAA",reviewMoreList);
    
    const { goodsId } = useParams<MatchParams>();
  //  const { reviewId } = useParams<MatchParams>();
    ...
    useEffect(() => {
     ...
     //   dispatch(fetchDetailReviewAddHelpNumSlice({reviewId}));
    ...
    const showMoreReviewBtn = () => { 
        ...
                dispatch(fetchDetailReviewMoreSlice({ids,goodsId}));
                if(reviewMoreList!=[]){
                        const showList = document.getElementsByClassName("remain")[0] as HTMLElement;
                        showList.style.display = "none";
                       
                        const closeList = document.getElementsByClassName("closeReviewMore")[0] as HTMLElement;
                                ...
                                return (
                                    ...
                                    <p className="g-reviewList_user"><b>{detailReviewData.nickName}</b>さん<span className="g-clip">{detailReviewData.commentDate}</span></p>
                                        ...
                                        <p className="g-reviewList_like">
                                                ...
                                                </i><span className="initialHelpNumSpan">参考になった{detailReviewData.reviewNum}人</span></a>
                                        </p>	
                                </li>	
                                );
                            })}
        ...
        <div className="g-foot-v g-foot-sm" id="js-review-more" aria-hidden="false">
            <p className="g-align-tc" id="showMoreReviewBtn">
                <a className="g-link displaymorereview" href="#p-reviewMore" role="button" aria-expanded="false" aria-controls="p-reviewMore" data-label="閉じる" data-accordion="{&quot;scroll&quot;:false}">
                    <i className="g-i g-i-arrow-d"></i><span id="showMoreLabel" onClick={showMoreReviewBtn} className="remain">レビューをもっと見る<span id="js-reviews-more"></span></span></a>
            </p>  
            {/* onClick={() => dispatch(fetchDetailReviewSlice(pageQa+1))} */}
            <p className="g-align-tc" id="closeBtn" >
                <a className="g-link displaymorereview" href="#p-reviewMore" role="button" aria-expanded="false" aria-controls="p-reviewMore" data-label="閉じる" data-accordion="{&quot;scroll&quot;:false}">
                    <i className="g-i g-i-arrow-d"></i>
                    <span id="closeLabel" onClick={showMoreReviewCloseBtn} className="closeReviewMore" style={{display:"none"}}>閉じる</span>
